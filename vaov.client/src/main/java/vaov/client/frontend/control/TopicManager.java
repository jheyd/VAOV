...
/**
 * This class manages the proposal part of this application: downloading,
 * uploading, showing, creating, editing propsals. Dies ist ein Versuch,
 * Abstimmungs- und Antragsanzeigetool voneinander zu trennen. Diese Klasse ist
 * darauf ausgelegt, allein ausfühbar und mittels Texteingaben steuerbar zu
 * sein.
 * 
 * @author dunkelzahn
 * 
...
public class TopicManager {
	public static final String[] commandNames = new String[] { "listTopics",
			"showTopic", "showIni", "pull" };
	public static final String[] commandShortNames = new String[] { "l", "t",
			"i", "p" };
	...
	private void buildTopicList() {
		...
		try {
			br = new java.io.BufferedReader(new java.io.FileReader(
					TopicManager.topicListFilePath));
	...
	private void listTopics() {
		...
		for (int i = 0; i < t.size(); i++)
			System.out.println("TOP" + String.valueOf(i) + ": "
					+ t.get(i).toString());
	}

	/**
	 * load TopicList data from server and write them to the file at
	 * topicListFilePath
	 */
	private void pull() {
	 ...
	 * 
	 * @param targetID
	 *            ID of the Ini to show, Format: "INI" + topicIndex + "." +
	 *            iniIndex
	 */
	private void showIni(String targetID) {
		...
		System.out.println(this.tops.getTopics().get(index1).getInis()
				.get(index2).toString());
	}
	 ...
	 * 
	 * @param topicID
	 *            ID of the Topic to show. Format: "TOP" + index
	 */
