...
import vaov.client.frontend.model.IniModel;
...
public class IniHandler {
	...
	public List<String> listTopics() {
		...
		for (int i = 0; i < t.size(); i++)
			l.add("TOP" + String.valueOf(i) + ": "
					+ t.get(i).getIniNames().get(0));
		return l;
	}
	 ...
	 * 
	 * @param topicID
	 *            the ID of the Topic to create the Ini in or "new" to create
	 *            the Ini in a new Topic
	 * @param name
	 ...
	 * @throws ParseException
	 */
	public void newIni(PrivateAccount acc, String topicID, String name,
		...
		try {
			...
			throw new ParseException("topicID has invalid format", 0);
		}

		try {
			MessageHandler.sendNewIni(name, text, topicID, acc);
