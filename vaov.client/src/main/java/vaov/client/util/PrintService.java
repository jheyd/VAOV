...
public class PrintService {
	...
	public static void print(OutputStream out, String hash) {
		...
		BoundingBox box = new BoundingBox(new Point(20, 20), new Dimension(
				(int) d.getWidth() - 20, 30));
		/*
		...
		try {
			child = box.getStringBounds("Bitte auf Papier Ausdrucken!",
					BoundingBox.HORIZ_ALIGN_LEFT, BoundingBox.VERT_ALIGN_TOP,
					fm, padding);
			child.drawWrappedString(g, fm, padding,
					BoundingBox.HORIZ_ALIGN_LEFT);
			child = box.getStringBounds("Bitte auf Papier Ausdrucken!",
					BoundingBox.HORIZ_ALIGN_RIGHT, BoundingBox.VERT_ALIGN_TOP,
					fm, padding);
			child.drawWrappedString(g, fm, padding,
		...
		box = new BoundingBox(new Point(20, 50), new Dimension(
				(int) d.getWidth() - 20, 150));
		/*
		 * g.drawRect((int)box.getAbsoluteLocation().getX(),
		 * (int)box.getAbsoluteLocation().getY(), (int)box.getSize().getWidth(),
		 * (int)box.getSize().getHeight());
		...
		g.setFont(id);
		fm = g.getFontMetrics();
		box = new BoundingBox(new Point(20, 250), new Dimension(
				(int) d.getWidth() - 20, 50));
		 ...
		 */
		try {
			child = box.getStringBounds("ID: " + hash,
					BoundingBox.HORIZ_ALIGN_LEFT, BoundingBox.VERT_ALIGN_TOP,
					fm, padding);
			child.drawWrappedString(g, fm, padding,
					BoundingBox.HORIZ_ALIGN_LEFT);
		...
		try {
			...
			hints.put(EncodeHintType.ERROR_CORRECTION, ErrorCorrectionLevel.H); // use
																				...
																				// highest
																				// possible
			bm = qw.encode(hash, BarcodeFormat.QR_CODE,
					(int) d.getWidth() - 100, (int) d.getWidth() - 100);
